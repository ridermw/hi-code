...
app/server/data/problems/reorder_list.json
...
You are given the head of a singly linked list. Reorder the list in-place to follow the pattern: L0 → Ln → L1 → Ln-1 → L2 → Ln-2 → … Do not modify node values; only change pointers.
