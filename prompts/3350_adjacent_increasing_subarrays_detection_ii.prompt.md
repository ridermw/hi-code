---
id: adjacent_increasing_subarrays_detection_ii
...
app/server/data/problems/adjacent_increasing_subarrays_detection_ii.json
...
Given an integer array `nums`, determine whether there exists an integer `k ≥ 2` and an index `i` such that: • `nums[i .. i + k - 1]` is strictly increasing • `nums[i + k .. i + 2k - 1]` is strictly increasing • the two subarrays are adjacent and of equal length Return `true` if such a pair exists, otherwise return `false`. This is a Hard sliding window problem because we must efficiently detect two back to back increasing windows without brute force.
