  ...
  it("handles basic fetch flows", async () => {
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          url.includes("/api/users/user-1/attempts") &&
          init?.method === "POST"
        ) {
          return jsonResponse(
            {
        ...
        if (
          url.includes("/api/users/user-1/reset") &&
          init?.method === "POST"
        ) {
          return jsonResponse({
            id: "user-1",
        ...
        if (url.endsWith("/api/flashcards")) {
          return jsonResponse({
            categories: [
              { id: "two_pointers", name: "Two Pointers", totalCards: 2 },
            ],
          });
        }
        ...
        if (
          url.includes("/api/users/user-1/flashcards/two_pointers") &&
          init?.method !== "POST"
        ) {
        ...
        if (
          url.includes("/api/users/user-1/flashcards/two_pointers/star") &&
          init?.method === "POST"
        ) {
    ...
    await expect(fetchProblem("two_sum")).resolves.toMatchObject({
      id: "two_sum",
    });
    await expect(
      submitAttempt("user-1", "two_sum", {
        algorithms: "alg-1",
        implementations: "impl-1",
        timeComplexities: "time-1",
        spaceComplexities: "space-1",
    ...
    await expect(resetUserProgress("user-1")).resolves.toMatchObject({
      attempts: {},
    });
    await expect(fetchFlashcardCategories()).resolves.toHaveLength(1);
    await expect(fetchFlashcards("two_pointers")).resolves.toMatchObject({
      category: { id: "two_pointers" },
    });
    await expect(
      fetchFlashcardStars("user-1", "two_pointers"),
    ).resolves.toEqual(["card-1"]);
    await expect(
      updateFlashcardStar("user-1", "two_pointers", "card-2", true),
    ).resolves.toContain("card-2");
  });
